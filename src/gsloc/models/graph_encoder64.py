...
class VPRGraphEncoder(nn.Module):
    ...
    def forward(self, batch):
        ...
        edge_attr = None
        if hasattr(batch, 'edge_attr') and batch.edge_attr is not None and False:
...
class MultiModalVPRGraphEncoder(nn.Module):
    def __init__(
        ...
        self.init_args = {
            ...
            "normalize": normalize,
            "graph_fusion_scale": graph_fusion_scale,
            "freeze_image_encoder": freeze_image_encoder,
            "train_only_aggregator": train_only_aggregator
        }
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, q_tokens, kv_tokens, key_padding_mask=None):
        """
        q_tokens:  [B, Nq, D]
        kv_tokens: [B, Nk, D]
        """
...
class GraphEnhancedMegaloc(nn.Module):
    def __init__(self,graph_encoder, image_encoder, graph_dim=256, cluster_dim=256, num_clusters=64):
        super().__init__()
        self.image_encoder = image_encoder
        self.graph_encoder = graph_encoder
        self.graph_proj = nn.Linear(graph_dim, cluster_dim)
        self.cross_attn = CrossAttentionBlock(d_model=cluster_dim, n_heads=8, dropout=0.1)
        self.normalize = True

        if image_encoder is not None:
            self.original_linear = image_encoder.aggregator.linear
    ...
    def forward(self, graph=None, image=None, mode="fusion", return_parts=False):
        ...
        if mode in ("graph", "fusion"):
            if graph is None:
                raise ValueError("graph is required for mode='graph' or 'fusion'")
            graph_z = self.graph_encoder(graph)
            out["graph"] = graph_z
        ...
        cluster_feats, t = self.image_encoder.aggregator(
            self.image_encoder.backbone(image),
            return_clusters=True  
        )   # cluster_feats: [B, 64, 256]

        # graph_emb = self.graph_proj(graph_z)  # [B, 256]
        graph_z_unsqueezed = graph_z.unsqueeze(1)  # [B, 1, 256]
        enhanced_clusters = self.cross_attn(cluster_feats, graph_z_unsqueezed)  # [B,64,256]

        enhanced_clusters = enhanced_clusters.permute(0, 2, 1)  # [B, 256, 64]
        enhanced_clusters_norm = F.normalize(enhanced_clusters, p=2, dim=1)  # [B, 256, 64]
        cluster_flat = enhanced_clusters_norm.flatten(1)  # [B, 16384]
    
        token_norm = F.normalize(t, p=2, dim=-1)  # [B, 256]

        combined = torch.cat([token_norm, cluster_flat], dim=1)  # [B, 16640]
        desc = self.original_linear(combined)
