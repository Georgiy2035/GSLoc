...
def _read_pose_matrix(pose_path: Path) -> np.ndarray:
    """Read a 4x4 pose matrix from a Replica `*.pose.txt` file."""
...
def iter_replica_frames(dataset_root: Union[str, Path]) -> Iterable[Tuple[str, Path, Path]]:
...
def build_replica_df(
        ...
        *,
        ...
        """Build a single `meta.parquet` for Replica.

        The resulting file contains:
        - `idx` (int): unique row id
        - `scene` (str): scene id (folder name under `data/`)
        ...
        for scene_id, img_path, pose_path in iter_replica_frames(root):
            try:
...
class Replica(Dataset):
    """Replica dataset that loads RGB frames from all scenes."""
    ...
    def __init__(
        ...
        *,
        ...
        if self.meta_path.exists() and not rebuild_meta:
            ...
            logger.info("Rebuilding metadata for replica dataset" if self.meta_path.exists() else "Metadata didn't found, rebuilding metadata for replica dataset")
            self.df = build_replica_df(self.dataset_root, limit=limit)
    ...
    def is_same_room_and_pose(
        ...
        *,
        ...
        """
        Return True iff:
        1) both samples belong to the exact same scene, and
        ...
        if scene_id_a is None or scene_id_b is None or scene_id_a != scene_id_b:
